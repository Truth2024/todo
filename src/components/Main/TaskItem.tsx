...
import { doneTask, updateTask } from "../../features/tasksSlice";
  ...
  return (
    <li className={`flex items-center ${style.list__item}`}>
      ...
      {isEditing ? (
        ...
        <h3
          className={`${style.title} ${task.done ? style.decoration : ''}`}
      ...
      <div className={`${style.icons} flex`}>
        {!isEditing && (
          <>
            ...
            <svg onClick={onDelete} width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              {/* иконка корзины */}
